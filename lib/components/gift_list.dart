...
class GiftListState extends State<GiftList> {
  ...
  Component build(BuildContext context) {
    return section(id: 'presentes', classes: 'gifts', [
      ...
      if (toastMessage case final message?)
        div(
          classes: toastIsError ? 'gift-toast is-error' : 'gift-toast',
          attributes: {'role': 'status', 'aria-live': 'polite'},
          [
            if (refreshing) span(classes: 'gift-toast-spinner', []),
            .text(message),
          ],
        ),
    ]);
  ...
  static List<StyleRule> get styles => [
    css('.gifts', [
      ...
      // Fica à esquerda do botão de atualizar: os dois são fixos no rodapé e
      // não podem se sobrepor no celular.
      css('.gift-toast', [
        css('&').styles(
